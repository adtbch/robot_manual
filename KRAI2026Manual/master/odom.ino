 ...
 * =====================================================================
 * FILE    : odom.ino
 * PERAN   : Record waypoint odom slave1 — zone1, approach, forest.
 *
 * SHARE + TOUCHPAD tahan 5 detik → mode record ON (tanpa reset)
 * R3 + L3 edge                    → keluar mode record
 * Kombinasi per slot (hanya di mode record) → poll + simpan + NVS
 *
...
void odomSaveZoneSlot(uint8_t idx) {
    if (idx >= ODOM_WP_COUNT) return;
    char key[8];
    Preferences prefs;
    prefs.begin(ODOM_NVS_NS, false);
    snprintf(key, sizeof(key), "z1_%u_x", idx);
    prefs.putFloat(key, gOdomWaypoints[idx].x_cm);
    snprintf(key, sizeof(key), "z1_%u_y", idx);
    prefs.putFloat(key, gOdomWaypoints[idx].y_cm);
    snprintf(key, sizeof(key), "z1_%u_w", idx);
    prefs.putFloat(key, gOdomWaypoints[idx].yaw_deg);
    snprintf(key, sizeof(key), "z1_%u_v", idx);
    prefs.putShort(key, gOdomWaypoints[idx].maxspeed_rpm);
    snprintf(key, sizeof(key), "z1_%u_ok", idx);
    prefs.putUChar(key, 1);
    prefs.end();
}

void odomLoadZoneSlot(uint8_t idx) {
    char key[8];
    Preferences prefs;
    prefs.begin(ODOM_NVS_NS, true);
    snprintf(key, sizeof(key), "z1_%u_ok", idx);
    if (prefs.getUChar(key, 0) == 0) {
        prefs.end();
        return;
    }
    OdomWaypoint& wp = gOdomWaypoints[idx];
    snprintf(key, sizeof(key), "z1_%u_x", idx);
...
void odomClearZoneSlotNvs(uint8_t idx) {
    char key[8];
    Preferences prefs;
    prefs.begin(ODOM_NVS_NS, false);
    snprintf(key, sizeof(key), "z1_%u_ok", idx);
...
void odomRequestRecord(OdomRecTarget target) {
    ...
    sendShowOdomCommand();
    Serial.printf("[OdomRecord] poll → target %u\n", static_cast<uint8_t>(target));
}

void odomEnterModeSave() {
    if (gOdomModeSave) return;
    gOdomModeSave = true;
    Serial.println("[OdomRecord] mode ON — R3+L3 keluar; combo record aktif");
}
...
void odomApplySample(OdomRecTarget target, float x_cm, float y_cm, float yaw_deg) {
    switch (target) {
        ...
        case OdomRecTarget::ZONE1_3: {
            ...
            wp.valid          = true;
            odomSaveZoneSlot(idx);
            Serial.printf("[OdomRecord] zone1 #%u x=%.1f y=%.1f yaw=%.1f\n",
                          idx + 1, wp.x_cm, wp.y_cm, wp.yaw_deg);
            break;
        }
...
void initOdomRec() {
    for (size_t i = 0; i < ODOM_WP_COUNT; i++) {
        odomLoadZoneSlot(static_cast<uint8_t>(i));
    }
    Serial.printf("[OdomRecord] NVS loaded zone1 %u/4 valid\n", odomZone1ValidCount());
}
...
void odomRecordClear() {
    for (size_t i = 0; i < ODOM_WP_COUNT; i++) {
        gOdomWaypoints[i] = {};
        odomClearZoneSlotNvs(static_cast<uint8_t>(i));
    }
    gOdomRecordPending = false;
    gOdomPendingTarget = OdomRecTarget::NONE;
}
...
uint8_t odomZone1ValidCount() {
    uint8_t n = 0;
    for (size_t i = 0; i < ODOM_WP_COUNT; i++) {
        if (gOdomWaypoints[i].valid) n++;
    }
    return n;
}

void odomRecordPrint(Print& out) {
    out.printf("OdomRecord zone1: %u/4 (mode=%s)\n",
               odomZone1ValidCount(), gOdomModeSave ? "ON" : "OFF");
