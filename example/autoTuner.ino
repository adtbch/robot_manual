...
namespace AutoTunerNS {
    ...
    constexpr float kTargetRpm = AUTOTUNE_TARGET_VEL * kRadPerSecToRpm;
...
void autoTunerTick(bool bootPressed) {
    ...
    case ATState::AT_MOTOR_INIT: {
        ...
        float kpMin = AutoTunerNS::kMinKpForMotionFraction * maxPwm / AUTOTUNE_TARGET_VEL;
        if (gCurrentKp < kpMin) gCurrentKp = kpMin;
        if (gCurrentKi < 0.1f) gCurrentKi = 0.5f;
        if (gCurrentKd < 0.0f) gCurrentKd = 0.0f;
    ...
    case ATState::AT_CYCLE_RUN: {
        ...
        if (millis() - gLastPidTickMs >= AutoTunerNS::kPidTickMs) {
            gLastPidTickMs = millis();
            convertEncoderToRPM();  // Update encoder values
